...
--2) Đếm tổng số khách hàng và tổng tiền của những khách hàng thuộc các quốc gia có mã vùng là US (lấy thông tin từ các bảng SalesTerritory, Sales.Customer, Sales.SalesOrderHeader, Sales.SalesOrderDetail). Thông tin bao gồm TerritoryID, tổng số khách hàng (countofCus), tổng tiền (Subtotal) với Subtotal = SUM(OrderQty*UnitPrice) 
	...
	select A.TerritoryID,COUNT(C.CustomerID) as CountofCus, SUM(D.OrderQty * D.UnitPrice) as Subtotal
	from Sales.SalesTerritory as A join Sales.Customer as C on A.TerritoryID = C.TerritoryID
								join Sales.SalesOrderHeader as H on H.CustomerID = H.CustomerID
...
--4) Liệt kê các sản phẩm (product)có đơn giá (unitPrice)<25 và số lượng bán trung bình >5, thông tin gồm ProductID, name,AverageofQty
	...
	select pro.ProductID, pro.Name, AVG(det.OrderQty) as AverageofQty
...
--5) Liệt kê các công việc (JobTitle) có tổng số nhân viên >20 người, thông tin gồm JobTitle, countofPerson=count(*) 
	...
	select JobTitle, count(BusinessEntityID) as CountofEmployee
...
--6) Tính tổng số lượng và tổng trị giá của các sản phẩm do các nhà cung cấp có tên kết thúc bằng ‘Bicycles’ và tổng trị giá >800000, thông tin gồm BusinessEntityID, Vendor_name, ProductID, sumofQty, SubTotal (sử dụng các bảng [Purchasing].[Vendor] [Purchasing].[PurchaseOrderHeader] và [Purchasing].[PurchaseOrderDetail])
	...
	select v.BusinessEntityID, v.Name, ProductID, SUM(OrderQty) as sumofQty , SUM(OrderQty * UnitPrice) as SubTotal
	from Purchasing.Vendor as v join Purchasing.PurchaseOrderHeader as h on h.VendorID = v.BusinessEntityID
							 join Purchasing.PurchaseOrderDetail as d on h.PurchaseOrderID = d.PurchaseOrderID
...
--7) Liệt kê các sản phẩm có trên 500 đơn đặt hàng trong quí 1 năm 2008 và có tổng trị giá >10000, thông tin gồm ProductID, Product_name, countofOrderID và Subtotal
	select p.ProductID, p.Name, COUNT(o.SalesOrderID) as countofOrderID , sum(OrderQty * UnitPrice) as  Subtotal 
	from Production.Product as p join Sales.SalesOrderDetail o on p.ProductID = o.ProductID
...
from [Person].[Person] as p join [Sales].[Customer] c on p.BusinessEntityID=c.CustomerID
...
select p.ProductID, Name,sum([OrderQty]) as  CountofOrderQty,year([OrderDate]) as yearofSale
from [Production].[Product] as p join [Sales].[SalesOrderDetail] d on p.ProductID=d.ProductID
...
from [HumanResources].[Department] as d join [HumanResources].[EmployeeDepartmentHistory] h on d.DepartmentID=h.DepartmentID
...
select 	c.CustomerID, COUNT(*) as CountofOrder
from Sales.Customer c join Sales.SalesOrderHeader h on c.CustomerID=h.CustomerID
group by c.CustomerID
having COUNT(*)>=all(select COUNT(*)
					 from Sales.Customer c join Sales.SalesOrderHeader h on c.CustomerID=h.CustomerID
					 group by c.CustomerID)	
-- 4) Liệt kê các sản phẩm (ProductID, Name) thuộc mô hình sản phẩm áo dài tay vớitên bắt đầu với “Long-Sleeve Logo Jersey”, dùng phép IN và EXISTS, (sử dụng bảng Production.Product và Production.ProductModel
select ProductID, Name
from Production.Product 
...
-- 9) Liệt kê các nhân viên không lập hóa đơn từ sau ngày 1/5/2008, thông tin gồmEmployeeID, FirstName, LastName (dữ liệu từ 2 bảng HR.Employees và Sales.Orders)
